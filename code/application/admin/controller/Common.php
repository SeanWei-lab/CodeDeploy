...
class Common extends Controller
{
    ...
    protected function ptaoput($ve, $bs)
    {
        if($this->actualDomain())
        {
            $this->assign($this->bd('Y2F0ZmlzaA=='), $this->bd('PGEgaHJlZj0iaHR0cDovL3d3dy4=').$ve['official'].'/" '.$this->bd('dGFyZ2V0PSJfYmxhbmsiIGlkPSJjYXRmaXNoIg==').$bs.'>'.$ve['name'].'&nbsp;'.$ve['description'].'&nbsp;'.$ve['number'].$this->bd('PC9hPiZuYnNwOyZuYnNwOw=='));
    ...
    protected function showdbbackup()
    {
        ...
        foreach($dbrecarr as $key => $val){
            ...
            $dbrecarr[$key] = [
                'path' => $val,
                'name' => 'catfishblog'.str_replace(['-', '_', ':', ' '], '', $bdate . '.cbb'),
                'date' => $bdate,
    ...
    {
        $blog_atr = Cache::get('blog_atr');
        if($blog_atr == false)
        {
            $author = self::getb('author');
            if(!empty($author))
            {
                $blog_atr = unserialize($author);
            }
            else
            {
                $blog_atr = '';
            }
            Cache::set('blog_atr',$blog_atr,3600);
        }
        if(empty($blog_atr))
        {
            return false;
        ...
        {
            if(isset($blog_atr['open']) && isset($blog_atr['veri']) && isset($blog_atr['rm']) && isset($blog_atr['sq']) && isset($blog_atr['ck']))
            {
                if($blog_atr['ck'] == md5($blog_atr['rm'].$blog_atr['sq']))
                {
                    return true;
                }
                else
