...
class Index extends Common
{
    public function index()
    {
        ...
        if(Request::instance()->has('page','post'))
        {
            ...
            if($getpage == false)
            {
                $data = Db::view('posts','id,post_keywords as guanjianzi,zuozhe,bianji,post_content as zhengwen,post_title as biaoti,post_excerpt as zhaiyao,post_modified as fabushijian,comment_count as pinglunshu,thumbnail as suolvetu,post_hits as yuedu,post_like as zan,istop as zhiding')
        ...
        if($catfishBlog == false)
        {
            ...
            $pageArr['meiye'] = $meiye;
            $pageArr['fenye'] = $fenye;
            $sx = $this->shangyiyexiayiye($dangqianye, $zongyeshu, Url::build('/'));
    ...
    public function article($id = 0)
    {
        if(empty($id) || $id == 'all')
        {
            if(Request::instance()->has('page','post'))
            {
                ...
                if($getpage == false)
                {
                    ...
                    $data = Db::view('posts','id,post_keywords as guanjianzi,zuozhe,bianji,post_content as zhengwenfz,post_title as biaoti,post_excerpt as zhaiyao,post_modified as fabushijian,comment_count as pinglunshu,thumbnail as suolvetu,post_hits as yuedu,post_like as zan,istop as zhiding')
                        ->view('users','user_nicename as nicheng,avatar as touxiang,sex as xingbie','users.id=posts.post_author')
                        ->where('post_status','=',1)
        ...
        else
        {
            ...
            $noArticle = false;
            $data = Db::view('posts','id,post_author,post_keywords as guanjianzi,zuozhe,bianji,post_source as laiyuan,post_content as zhengwen,post_title as biaoti,post_excerpt as zhaiyao,post_status,comment_status,post_modified as fabushijian,post_type,comment_count as pinglunshu,thumbnail as suolvetu,post_hits as yuedu,post_like as zan,istop as zhiding,fujian,fujianurl')
                ->view('users','user_login as yonghu,user_nicename as nicheng,avatar as touxiang,sex as xingbie','users.id=posts.post_author')
                ->where('posts.id',$id)
                ->where('status','=',1)
                ->find();
            ...
            if(isset($data['comment_status']) && $data['comment_status'] == 1 && $closeComment == 0)
            {
                $pinglun = Db::name('comments')
                    ...
                    ->field('c.id,c.createtime as shijian,c.content as neirong,c.parent_id,u.user_login,u.user_nicename as nicheng,u.user_email as email,u.user_url as url,u.avatar as touxiang,u.signature as qianming')
                    ->paginate($this->everyPageShows);
                ...
                if(!empty($idStr)){
                    $zpinglun = Db::name('comments')
                        ->alias('c')
                        ->where('c.topid','in',$idStr)
                        ->where('c.status','=',1)
                        ->order('c.id asc')
                        ->join('users u','c.uid = u.id','LEFT')
                        ->field('c.id,c.createtime as shijian,c.content as neirong,c.parent_id,u.user_login,u.user_nicename as nicheng,u.user_email as email,u.user_url as url,u.avatar as touxiang,u.signature as qianming')
        ...
        if(Request::instance()->has('page','post'))
        {
            ...
            if($getpage == false)
            {
                ...
                $data = Db::name('posts p')
                    ...
                    ->field('p.id,t.term_id,p.post_keywords as guanjianzi,p.zuozhe,p.bianji,p.post_content as zhengwenfz,p.post_title as biaoti,p.post_excerpt as zhaiyao,p.post_modified as fabushijian,p.post_type as type,p.comment_count as pinglunshu,p.thumbnail as suolvetu,p.post_hits as yuedu,p.post_like as zan,p.istop as zhiding,u.user_login as yonghu,u.user_nicename as nicheng,u.avatar as touxiang,u.sex as xingbie')
            ...
            if(!empty($catid))
            {
                ...
                if($getpage == false)
                {
                    $data = Db::view('term_relationships','term_id')
                        ->view('posts','id,post_keywords as guanjianzi,zuozhe,bianji,post_content as zhengwenfz,post_title as biaoti,post_excerpt as zhaiyao,post_modified as fabushijian,post_type as type,comment_count as pinglunshu,thumbnail as suolvetu,post_hits as yuedu,post_like as zan,istop as zhiding','posts.id=term_relationships.object_id')
                        ->view('users','user_nicename as nicheng,avatar as touxiang,sex as xingbie','users.id=posts.post_author')
                        ->where('term_id','in',$termid)
                        ->where('post_status','=',1)
