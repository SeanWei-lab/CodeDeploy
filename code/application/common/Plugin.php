...
class Plugin extends Controller
{
    ...
    protected function getPost($param = '', $isdecode = true)
    {
        if($param == '')
        {
    ...
    protected function getGet($param = '', $isdecode = true)
    {
        if($param == '')
        {
    ...
    protected function uploadFile($type = '', $place = 'uploads', $upfile = 'file', $original = false)
    {
        $fj = '';
        if(empty($place)){
            $place = 'uploads';
    ...
    protected function uploadFiles($type = '', $place = 'uploads', $upfile = 'file', $original = false)
    {
        $fj = '';
        if(empty($place)){
            $place = 'uploads';
    ...
    {
        $session_prefix = 'catfish'.str_replace(['/','.',' ','-'],['','?','*','|'],Url::build('/'));
        if(Session::has($session_prefix.'user_id')){
            return true;
        }
        return false;
    }
    ...
    {
        $session_prefix = 'catfish'.str_replace(['/','.',' ','-'],['','?','*','|'],Url::build('/'));
        return Session::has($session_prefix.$name);
    }
    protected function setSession($name, $val)
    {
        $session_prefix = 'catfish'.str_replace(['/','.',' ','-'],['','?','*','|'],Url::build('/'));
        Session::set($session_prefix.$name,$val);
    }
    protected function getSession($name)
    {
        $session_prefix = 'catfish'.str_replace(['/','.',' ','-'],['','?','*','|'],Url::build('/'));
        return Session::get($session_prefix.$name);
    }
    protected function delSession($name)
    {
        $session_prefix = 'catfish'.str_replace(['/','.',' ','-'],['','?','*','|'],Url::build('/'));
        Session::delete($session_prefix.$name);
    }
    ...
    {
        $pluginName = $this->getPlugin();
        $path = APP_PATH.'plugins'.DS.$pluginName.DS;
        if(is_array($files)){
            foreach($files as $file){
                if(stripos($file, $path) === false){
                    $file = $path . ltrim(str_replace(['/', '\\'], DS, $file), DS);
                }
                require_once $file;
            }
        }
        else{
            if(stripos($files, $path) === false){
                $files = $path . ltrim(str_replace(['/', '\\'], DS, $files), DS);
            }
            require_once $files;
        }
    }
