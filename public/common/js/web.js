...
$(document).ready(function(){
    ...
    $("#recordqueding").on("click", function(event){
        ...
        if($("#icpming").val() != "" && $("#icphao").val() != ""){
            recordicp = "<a href='http://beian.miit.gov.cn/' target='_blank'>" + $("#icpming").val() + "ICP备 " + $("#icphao").val() + "号</a>";
        }
        if($("#gonganming").val() != "" && $("#gonganhao").val() != ""){
            recordgg = "<a href='http://www.beian.gov.cn/portal/registerSystemInfo?recordcode=" + $("#gonganhao").val() + "' target='_blank'><img src='" + $("#webroot").text() + "public/common/images/images/gongan.png'> " + $("#gonganming").val() + "公网安备 " + $("#gonganhao").val() + "号</a>";
        }
